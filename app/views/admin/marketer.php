...
<body>
    ...
    <div class="container-fluid mt-4">
        <div class="row p-3 py-3">
            <div class="bg-dark rounded mb-4">
                <h3 class="fw-bolder text-white mb-0 p-3 shadow-lg">Marketers' List</h3>
            </div>
            <?php
            foreach ($marketer as $mkt) {
            ?>
                <div class="col-4 p-4">
                    <div class="shadow card border-0 bg-white my-2 mx-4">
                        <div class="row col m-0 p-0 bg-dark rounded-3">
                            <a onclick="sendData(this.id)" id="<?= $mkt['admin_id']; ?>" href="#" data-bs-toggle="modal" data-bs-target="#updateMarketer"><i class="fad fa-user-edit text-info text-end p-3"></i></a>
                        </div>
                        <div class="col-12 text-center p-4 ui-widget-shadow">
                            <img id="marketerImg<?= $mkt['admin_id'] ?>" class="rounded-circle shadow" src="<?php echo BASE_URL . PUBLIC_DIR; ?>/images/marketer/<?= $mkt['admin_id']; ?>" alt="Marketer name" style="height: 2.2in; width: 2.2in; object-position: left; object-fit:cover;">
                            <div id="Mid<?= $mkt['admin_id'] ?>" class="fs-3 fw-bolder text-dark pt-3"><?= $mkt['admin_id']; ?></div>
                        </div>
                        <div class="col-12 py-3" style="box-shadow: inset 0px 1px 8px -7px black;">
                            <div class="d-flex justify-content-center">
                                <h3 id="fname<?= $mkt['admin_id'] ?>"><?= $mkt['admin_fname']; ?></h3>&nbsp
                                <h3 id="mname<?= $mkt['admin_id'] ?>"><?= $mkt['admin_mname']; ?></h3>&nbsp
                                <h3 id="lname<?= $mkt['admin_id'] ?>"><?= $mkt['admin_lname']; ?></h3>
                            </div>
                            <h5 id="username<?=$mkt['admin_id']?>" class="text-center text-black-50 mb-4"><?= $mkt['admin_username']; ?></h5>
                            <div class="row px-4 gy-3">
                                <div class="col-12"><b>Email:&nbsp</b><span id="email<?=$mkt['admin_id']?>"><?= $mkt['admin_email']; ?></span></div>
                                <div class="col-12"><b>Number:&nbsp</b><span id="contact<?= $mkt['admin_id'] ?>"><?= substr_replace(substr_replace($mkt['admin_cnum'], '-', 4, 0), '-', 8, 0) ?></span></div>
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <div class="container-fluid p-4">
                            <div class="row">
                                ...
                                <div class="input-group mb-3">
                                    <input type="file" class="form-control" id="marketerImg" name="marketerImg" required>
                                    <label class="input-group-text text-white bg-dark bg-gradient" for="marketerImg">Upload</label>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="updateMarketer" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <form action="<?= site_url('Admin/Update_Marketer') ?>" enctype="multipart/form-data" method="POST">
                <input type="text"  name="Uid" id="Uid" hidden>
                    <div class="modal-body p-0">
                        <div class="text-white d-flex justify-content-between m-0 p-3 mb-4 bg-dark bg-gradient">
                            <div></div>
                            <h5 class="modal-title fw-bold" id="staticBackdropLabel">Update Marketer</h5>
                            <i class="fas fa-times-circle align-content-center" data-bs-dismiss="modal" aria-label="Close" style="font-size: 1.5rem; cursor:pointer"></i>
                        </div>
                        <div class="container-fluid p-4">
                            <div class="row">
                                ...
                                <!-- MARKETER IMAGE -->
                                <label for="UproductImg" class="pt-2 ps-3">Marketer Image:</label>
                                <div class="input-group mb-3">
